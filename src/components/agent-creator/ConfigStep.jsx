...
export const LAYOUT_OPTIONS = [
  {
    ...
    blueprint: 'Follow SIDEBAR + FILTER + TABLE LAYOUT — REQUIRED SHAPE in the system prompt: header-action with a Filters toggle button, sidebar with flyout-card+filter-panel in Left and a single table in Default. Fill in real columns matching the flow\'s transformTable output field names and a real Filters Init round-trip. Do NOT add KPI tiles, charts, tabs, or a card grid — this layout is filter panel + one table, nothing else.',
  },
  {
    id: 'kpi-charts-table',
    icon: '📊',
    ...
    blueprint: 'This has charts/KPIs, so follow CHART/KPI LAYOUTS — DESIGN FREELY in the system prompt: design the composition yourself (a filter sidebar if useful, a KPI tiles row, one or two charts, a detail table — in whatever arrangement best fits the data), but the functional contracts listed there (real chart Init/highchartsOptions/seriesMappings, real KPI tile pattern, filter-panel Init contract, table ShowFilter/Columns) are mandatory regardless of composition. Do NOT add a tab-group — that belongs to the Tabbed Dashboard layout, not this one.',
  },
  {
    id: 'tabbed-dashboard',
    icon: '🗂️',
    ...
    blueprint: 'This has tabs/charts/KPIs, so follow CHART/KPI LAYOUTS — DESIGN FREELY in the system prompt: design a tab-group keyed by the agent\'s real distinct breakdowns, each tab holding its own KPI/chart/table combination — composition is yours to decide, but the functional contracts listed there (chart/KPI/table/filter-panel correctness) are mandatory regardless. A tab-group is required — do not flatten this into a single non-tabbed view.',
  },
  {
    id: 'card-grid',
    icon: '🗃️',
    ...
    blueprint: 'A grid container of repeating card nodes bound to a listOfMap dataset (one card per row), each card showing a few key-value fields for that record. If a filter sidebar is needed, follow SIDEBAR + FILTER + TABLE LAYOUT for the header-action/sidebar structure, with the card grid in place of the table in the sidebar\'s Default slot. Optional small KPI/chart summary row above the grid is allowed (follow CHART/KPI LAYOUTS\' functional contracts for that part) — but do NOT add a plain data table or tabs; the card grid is the primary content, not a table.',
  },
  {
    id: 'master-detail',
    icon: '🔍',
