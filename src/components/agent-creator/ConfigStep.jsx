...
const LAYOUT_OPTIONS = [
  {
    ...
    blueprint: 'Follow the CANONICAL SIDEBAR + FILTER + CONTENT SKELETON, VARIANT 1 (filter panel + single table) in the system prompt EXACTLY — same Container types, same slot names (header-action Left/Right, sidebar Left/Default/Right), same nesting, same gap/spacing. Fill in real columns matching the flow\'s transformTable output field names and a real Filters Init round-trip; do not restructure the skeleton.',
  },
  {
    id: 'kpi-charts-table',
    icon: '📊',
    ...
    blueprint: 'Follow the CANONICAL SIDEBAR + FILTER + CONTENT SKELETON, VARIANT 2 (filter panel + tab-group) in the system prompt, but with a single tab (or no tab-group at all if only one view) whose content starts with a grid row of 2-4 KPI tiles (card + two key-value elements: label row, bold value row bound to a scalar), then one or two chart cards, then a detail table — same Container types/slot names/nesting as the skeleton otherwise.',
  },
  {
    id: 'tabbed-dashboard',
    icon: '🗂️',
    ...
    blueprint: 'Follow the CANONICAL SIDEBAR + FILTER + CONTENT SKELETON, VARIANT 2 (filter panel + tab-group) in the system prompt EXACTLY — same Container types, same slot names, same nesting/spacing. Replace the placeholder tab names with the agent\'s real distinct breakdowns; each tab\'s content holds its own KPI/chart/table combination relevant to that tab specifically.',
  },
  {
    id: 'card-grid',
    icon: '🗃️',
    ...
    blueprint: 'A grid container of repeating card nodes bound to a listOfMap dataset (one card per row), each card showing a few key-value fields for that record. If a filter sidebar is needed, use CANONICAL SIDEBAR + FILTER + CONTENT SKELETON VARIANT 1\'s sidebar/header-action structure exactly, with the card grid in place of the table in Slots.Default. Optional small KPI/chart summary row above the grid.',
  },
  {
    id: 'master-detail',
    icon: '🔍',
    ...
    blueprint: 'Follow the CANONICAL SIDEBAR + FILTER + CONTENT SKELETON, VARIANT 1 EXACTLY, including the Slots.Right stack host (do not omit it for this layout — it is required here). The table\'s columns include an action-button "Insights" column (Events.Triggers.OnClick -> {EventId:"push-details-flyout", ContainerId:"details-button"}) wired to that stack host\'s Push/Pop listeners.',
  },
  {
    id: 'none',
    icon: '💬',
