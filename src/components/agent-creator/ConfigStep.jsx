...
const LAYOUT_OPTIONS = [
  ...
  },
  {
    ...
    blueprint: 'Follow SIDEBAR + FILTER + CONTENT LAYOUT — REQUIRED SHAPE in the system prompt (header-action + sidebar with flyout-card+filter-panel), with the sidebar\'s Default slot holding a grid row of 2-4 KPI tiles (card + two key-value elements: label row, bold value row bound to a scalar), then one or two chart cards, then a detail table.',
  },
  {
    id: 'tabbed-dashboard',
    icon: '🗂️',
    ...
    blueprint: 'Follow SIDEBAR + FILTER + CONTENT LAYOUT — REQUIRED SHAPE in the system prompt, with the sidebar\'s Default slot holding a tab-group keyed by the agent\'s real distinct breakdowns; each tab\'s content holds its own KPI/chart/table combination relevant to that tab specifically.',
  },
  {
    id: 'card-grid',
    icon: '🗃️',
    ...
    blueprint: 'A grid container of repeating card nodes bound to a listOfMap dataset (one card per row), each card showing a few key-value fields for that record. If a filter sidebar is needed, follow SIDEBAR + FILTER + CONTENT LAYOUT — REQUIRED SHAPE for the header-action/sidebar structure, with the card grid in place of the table in the sidebar\'s Default slot. Optional small KPI/chart summary row above the grid.',
  },
  {
    id: 'master-detail',
    icon: '🔍',
